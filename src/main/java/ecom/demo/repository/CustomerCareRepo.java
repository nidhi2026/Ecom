...
public class CustomerCareRepo {
    ...
    public int addCustomerCare(CustomerCare customerCare) {
        String sql = "INSERT INTO CustomerCare (customerCareID, userID, priority, issue, description, status, filedTime) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try {
            System.out.println("customerCareID: " + customerCare.getCustomerCareID());
            System.out.println("userID: " + customerCare.getUserID());
            System.out.println("priority: " + customerCare.getPriority().name());
            System.out.println("issue: " + customerCare.getIssue());
            System.out.println("description: " + customerCare.getDescription());
            System.out.println("status: " + customerCare.getStatus().name());
            System.out.println("filedTime: " + customerCare.getFiledTime());
    
            return jdbcTemplate.update(sql, customerCare.getCustomerCareID(), customerCare.getUserID(), 
                customerCare.getPriority().name(), customerCare.getIssue(), 
                customerCare.getDescription(), customerCare.getStatus().name(), 
                customerCare.getFiledTime());
        } catch (Exception e) {
            System.out.println("Error adding customer care: " + e.getMessage());
            return 0;
        }
