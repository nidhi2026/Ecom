...
public class ProductImageRepo {
    ...
    public int addProductImage(ProductImage productImage) {
        String sql = "INSERT INTO ProductImages (productID, imageLink) VALUES (?, ?)";
        try {
    ...
    public ProductImage getProductImageById(UUID productID) {
        String sql = "SELECT * FROM ProductImages WHERE productID = ?";
        try {
            return jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(ProductImage.class), productID.toString());
        } catch (Exception e) {
    ...
    public int updateProductImage(ProductImage productImage) {
        String sql = "UPDATE ProductImages SET imageLink = ? WHERE productID = ?";
        try {
