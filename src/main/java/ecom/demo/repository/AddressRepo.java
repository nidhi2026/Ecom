...
public class AddressRepo {
    ...
    public int addAddress(Address address) {
        String sql = "INSERT INTO Address (addressID, userID, streetName, cityName, districtName, pincode, countryName) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try {
    ...
    public int deleteMultipleAddresses(List<String> addressIDs) {
        String sql = "DELETE FROM Address WHERE addressID IN (?)";
        try {
            // Convert list of addresses into a comma-separated string
            String joinedAddressIDs = String.join(",", addressIDs);
            return jdbcTemplate.update(sql, joinedAddressIDs);
        } catch (Exception e) {
            System.out.println("Error deleting multiple addresses: " + e.getMessage());
            return 0;
        }
    }
    ...
    public List<Address> getAllAddresses(UUID userID) {
        ...
        try {
            return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(Address.class), userID.toString());
        } catch (Exception e) {
    ...
    public List<Address> getAddressesByPersonID(String personID) {
        ...
        try {
            return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(Address.class), personID);
        } catch (Exception e) {
            System.out.println("Error fetching addresses by person ID: " + e.getMessage());
            return List.of(); // Return empty list if an error occurs
        }
    }
    ...
    public int removeAddress(UUID addressID) {
